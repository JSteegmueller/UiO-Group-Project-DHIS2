...
import { CircularLoader, DropdownButton, MenuItem, FlyoutMenu } from "@dhis2/ui";
...
function RequestCommodity({ commodityValue }) {
    ...
    const [commodityValues, setCommodityValues] = useState([commodityValue[0], commodityValue[1]]);
    ...
    const listItems = commodityValue[3].map((value, index) => {
        return (
            <MenuItem
                ...
                label={value[0]}
                onClick={() => {
                    getCommodity([value[0], value[1].id]);
                }}
            />
        );
    });
